...
`PATCH /blog/community/comments/<id>/pin`，沿用 `X-Admin-Secret` 鉴权。
请求体：`{"is_pinned": true}`（取消时为 `false`，必须是布尔值）。
仅可操作已发布留言；无管理员权限返回 401，参数无效返回 400，留言不存在或未发布返回 404。
成功返回 `success`、`comment_id`、`is_pinned`，响应禁止缓存。
公开留言增加 `is_pinned` 布尔字段；拒绝、删除或转为反馈会取消置顶。
读取留言默认选择最新 300 条（limit 最大 500），另外补齐公开置顶及可见父级链，最终按 id 升序返回；因此总条数可超过 limit。
主页只在群公告下汇总置顶，不调整聊天顺序；博客将置顶留言（包括回复）提到最前，并避免重复显示。
现有数据库在初始化时自动添加 is_pinned 字段。部署时先更新并重启后端，再更新两个前端。


### 社区删除记录保留与懒清理

普通评论树删除为软删除：设置 `status = deleted`、清除置顶，并为每个成员记录 UTC `deleted_at`；重复删除不重置已有删除时间。旧版已删除且缺少时间戳的普通评论在迁移时以当时的 UTC 时间开始保留期，不使用创建时间倒推。带 `moved_to_feedback_id` 的迁入反馈记录不补记删除时间，也不参与普通评论清理。

每次普通评论、反馈聊天室留言、反馈主题回复或新反馈主题及其首条消息成功提交后（包括待审核或拒绝记录），通过统一的失败隔离入口触发独立事务中的懒清理检查；内部 `status = deleted` 写入、读取、点赞和评论转为反馈不触发。所有入口共用同一个每日标记。`community_metadata.last_purge_date` 持久化 UTC 日期，多进程通过 `BEGIN IMMEDIATE` 串行检查，每个 UTC 日最多尝试一次。清理失败会回滚清理改动并记录日志，保留当天尝试标记，次日有新留言时再尝试，不影响已经接受的评论；若连清理事务或标记都无法提交，则无法保证保存当天标记。

保留期为删除时间加三个日历月，时刻按 UTC 保留，目标月无对应日期时夹到月末（如 11 月 30 日 12:00 到次年 2 月 28/29 日 12:00），到达该时刻即符合清理条件。只有整棵根评论树的所有成员均已删除且到期、未迁入反馈、未被反馈来源引用时，才物理删除整树；仍有存活成员的树继续保留。无效时间戳保守保留。

反馈主题根据已有 `deleted_at` 使用相同保留期，到期后连同所属消息、来源快照和事件记录清理。仍被评论 `moved_to_feedback_id` 或本轮未清理主题的 `merged_into_id` 引用的主题继续保留，以保护引用完整性。反馈聊天室消息不在本次清理范围内。

这里保留的是同一数据库内的软删除记录，并非独立备份；清理会物理删除符合条件的记录，不会自动创建、读取或删除外部备份文件。没有新留言时不会定时执行清理。成功清理提交后输出 INFO 日志，仅包含各表物理删除条数（评论、反馈主题、反馈消息、来源快照和事件），不含留言内容或身份信息；当天跳过和失败回滚不输出成功数量日志。


`server.py` 在初始化时为 `community` logger 配置唯一的 INFO stderr handler（`sleepy.community.stderr`），直接运行和导入 WSGI 应用均生效；由进程管理器收集标准错误流。根 logger、根 handler 及其他模块的日志级别保持不变。该 logger 不向根传播，避免根 handler 的 WARNING 门槛屏蔽计数或重复输出；服务端接管此 logger 的直接 handler，移除旧绑定但不关闭可能被其他 logger 共用的资源。重复初始化/模块重载复用同一个具名 handler。独立导入 `community.py` 不修改宿主日志配置。启动回归测试见 `tests/test_community_logging.py`，使用独立进程、临时数据库和真实 Waitress 日志初始化（拦截网络服务创建），无需 `assertLogs` 临时提升级别。
