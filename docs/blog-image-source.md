...
旧/images/projects/...接口302跳转博客，Cache-Control max-age=300，兼容缓存页面与旧客户端。本地 images 文件读取、目录配置与启动自动建目录逻辑已移除；其他 /images/ 路径返回 404。已停用 SLEEPY_IMAGES_DIR，旧环境变量可删除。音乐及封面服务不受影响。图片是否存在由博客响应决定，不逐图HEAD检查，不下载或代理图片内容。

当前返回博客原图而不是指纹WebP；先解决双份维护。后续可缓存博客/_images/manifest.json选取展示图，查找失败回退原图。不要手动猜测WebP文件名。

发布前确认SLEEPY_BLOG_BASE_URL正确，博客/images/projects路径可从主页加载，再验证/blog-posts两个字段和旧接口302。旧项目入口不依赖本地文件；发布包继续排除 images/，避免重新打包历史素材。
